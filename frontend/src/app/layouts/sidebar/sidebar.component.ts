...
export class SidebarComponent implements OnInit {
  ...
  constructor(private router: Router, public translate: TranslateService , private service:UserserviceService) {
    translate.setDefaultLang('en');
  }

  ngOnInit(): void {
    // Get user role (Assuming you have a service to fetch this)
    const userRole = this.service.getUserRoles();
  
    // Filter menu items based on role
    this.menuItems = this.filterMenuItemsByRole(MENU, userRole);
...
filterMenuItemsByRole(menuItems: MenuItem[], role: string[]): MenuItem[] {
  return menuItems
    .map(item => {
      if (item) {
        const newItem = { ...item }; // Make a copy of the item

        // Recursively filter sub-items if they exist
        if (newItem.subItems) {
          newItem.subItems = this.filterMenuItemsByRole(newItem.subItems, role);
        }

        // Return item if it has no role or if the current role is allowed
        if (!newItem.role || newItem.role.includes(role)) {
          return newItem;
        }
      }

      // If the item is null or the role doesn't match, return null
      return null;
    })
    .filter(item => item !== null) as MenuItem[]; // Filter out null values and cast as MenuItem[]
}
